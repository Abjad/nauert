...
def test_BeatwiseQSchemaItem___new___03():
    item = nauert.BeatwiseQSchemaItem(beatspan=abjad.ValueDuration(1, 8))
    assert item.beatspan == abjad.ValueDuration(1, 8)
    assert item.search_tree is None
    assert item.tempo is None


def test_BeatwiseQSchemaItem___new___04():
    metronome_mark = abjad.MetronomeMark(abjad.ValueDuration(1, 4), 57)
    item = nauert.BeatwiseQSchemaItem(
        beatspan=abjad.ValueDuration(1, 8), tempo=metronome_mark
    )
    assert item.beatspan == abjad.ValueDuration(1, 8)
    assert item.search_tree is None
    assert item.tempo == metronome_mark
