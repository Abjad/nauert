...
def test_BeatwiseQSchema___init___01():
    ...
    item_c = nauert.BeatwiseQSchemaItem(beatspan=abjad.ValueDuration(1, 8))
    ...
    schema = nauert.BeatwiseQSchema(
        ...
        tempo=abjad.MetronomeMark(abjad.ValueDuration(1, 16), 32),
    )
    assert len(schema.items) == 3
    assert schema.beatspan == abjad.ValueDuration(1, 32)
    assert schema.search_tree == nauert.UnweightedSearchTree({3: None})
    assert schema.tempo == abjad.MetronomeMark(abjad.ValueDuration(1, 16), 32)


def test_BeatwiseQSchema___init___02():
    schema = nauert.BeatwiseQSchema()
    assert len(schema.items) == 0
    assert schema.beatspan == abjad.ValueDuration(1, 4)
    assert schema.search_tree == nauert.UnweightedSearchTree()
    assert schema.tempo == abjad.MetronomeMark(abjad.ValueDuration(1, 4), 60)
